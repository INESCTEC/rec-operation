...
from rec_op_lem_prices.optimization.structures.I_O_stage_2_pool_milp_evs import (
	INPUTS_S2_POOL_EVS,
	OUTPUTS_S2_POOL_EVS,
	INPUTS_S2_DUAL_EVS,
	OUTPUTS_S2_DUAL_EVS
...
def test_solve_collective_pool_milp():
	...
	milp = StageTwoMILPPool(INPUTS_S2_POOL_EVS)
	...
	for ki, valu in results.items():
		assert valu == OUTPUTS_S2_POOL_EVS.get(ki), f'{ki}'


def test_solve_collective_dual_milp():
	# Assert the creation of a correct class
	milp = StageTwoMILPPool(INPUTS_S2_DUAL_EVS)
	assert isinstance(milp, StageTwoMILPPool)
	...
	for ki, valu in results.items():
		assert valu == OUTPUTS_S2_DUAL_EVS.get(ki), f'{ki}'
