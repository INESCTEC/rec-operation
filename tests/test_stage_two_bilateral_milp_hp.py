...
from rec_op_lem_prices.optimization.structures.I_O_stage_2_bilateral_milp_hp import (
	INPUTS_S2_BILATERAL_HP,
	OUTPUTS_S2_BILATERAL_HP
)

def test_solve_collective_pool_milp():
	# Assert the creation of a correct class
	milp = StageTwoMILPBilateral(INPUTS_S2_BILATERAL_HP)
	...
	for ki, valu in results.items():
		assert valu == OUTPUTS_S2_BILATERAL_HP.get(ki), f'{ki}'
