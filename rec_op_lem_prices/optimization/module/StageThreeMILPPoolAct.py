...
class StageThreeMILPPoolAct:
	def __init__(self, backpack: BackpackS3PoolDict, solver=SOLVER, timeout=TIMEOUT, mipgap=MIPGAP):
		...
		self._flex_act = backpack.get('flex_act')  # activated flexibility [kWh]
	...
	def __define_milp(self):
		...
		for n in self.set_meters:
			...
			if meter_btm_ev is not None:
				...
				self._min_energy_storage_ev[n] = {ev: meter_btm_ev[ev]['min_energy_storage_ev'] for ev in self.sets_btm_ev[n]}
		...
		for t in self.time_series:
			for n in self.set_meters:
				if t != 0:
					objective += lpSum(hvac_cost_comfort[n][h][t] * 2 for h in self.set_hvac[n]) + lpSum(self.hp_cost_comfort[n][hp][t] * 2 for hp in self.set_hp[n])
		...
		# Eq.
		self.milp += \
			lpSum( (lpSum(e_flex[n][t]) for n in self.set_meters) for t in self.time_series) == self._flex_act, \
			'Flex_act_'
		...
		for n, t in itertools.product(self.set_meters, self.time_series):
			...
			for ev in self.sets_btm_ev[n]:
				...
				# Eq. 41
				if t == 0:
					self.milp += ev_stored[n][ev][t] == self._init_e_ev[n][ev] + self._eff_bc_ev[n][ev] * p_ev_charge[n][ev][t] \
								  - (1 / self._eff_bd_ev[n][ev]) * p_ev_discharge[n][ev][t] \
								 - self._trip_ev[n][ev][t], \
								 'EV_balance_' + increment
				else:
					self.milp += ev_stored[n][ev][t] == ev_stored[n][ev][t - 1] + self._eff_bc_ev[n][ev] * p_ev_charge[n][ev][
						t] - (1 / self._eff_bd_ev[n][ev]) * p_ev_discharge[n][ev][t] - self._trip_ev[n][ev][t], \
								 'EV_balance_' + increment
			...
			for n, t in itertools.product(self.set_meters, self.time_series):
				for h in self.set_hvac[n]:
					...
					if self.type[n][h] == 'inverter':
						...
						self.milp += hvac_power[n][h][t] == 0.2 * delta_hvac[n][h][t] * self.hvac_capacity[n][h], f'HVAC_Power_Level_' + increment

					if self.type[n][h] == 'state':
						if t != 0:
							self.milp += (hvac_mode_heat[n][h][t] + hvac_mode_off[n][h][t] + hvac_mode_cool[n][h][t] == 1 ), f'HVAC_mode_sum_' + increment
							# Heating mode
							self.milp += (hvac_temp[n][h][t] <= (
										...
										+ self._big_m * (1 - hvac_mode_heat[n][h][t])), f'HVAC_Heating_UB_state' + increment
							self.milp += (hvac_temp[n][h][t] >= (
										self.T_out[n][h][t] + hvac_power[n][h][t] * self.thermal_resist[n][h] - (
										self.T_out[n][h][t] + hvac_power[n][h][t] * self.thermal_resist[n][h] -
										hvac_temp[n][h][t - 1]) * math.exp(
										- self._delta_t / (self.thermal_resist[n][h] * self.thermal_cap[n][h])))
										- self._big_m * (1 - hvac_mode_heat[n][h][t])), f'HVAC_Heating_LB_state' + increment
							...
							self.milp += (hvac_temp[n][h][t] <= (
										...
										hvac_temp[n][h][t - 1]) * math.exp(
										- self._delta_t / (self.thermal_resist[n][h] * self.thermal_cap[n][h])))
										+ self._big_m * (1 - hvac_mode_off[n][h][t])), f'HVAC_Off_UB_state' + increment
							self.milp += (hvac_temp[n][h][t] >= (
										self.T_out[n][h][t] - (
										self.T_out[n][h][t] -
										hvac_temp[n][h][t - 1]) * math.exp(
										- self._delta_t / (self.thermal_resist[n][h] * self.thermal_cap[n][h])))
										- self._big_m * (1 - hvac_mode_off[n][h][t])), f'HVAC_Off_LB_state' + increment
							...
							self.milp += (hvac_temp[n][h][t] <= (
										...
										hvac_temp[n][h][t - 1]) * math.exp(
										- self._delta_t / (self.thermal_resist[n][h] * self.thermal_cap[n][h])))
										+ self._big_m * (1 - hvac_mode_cool[n][h][t])), f'HVAC_Cooling_UB_state' + increment
							self.milp += (hvac_temp[n][h][t] >= (
										self.T_out[n][h][t] - hvac_power[n][h][t] * self.thermal_resist[n][h] - (
										self.T_out[n][h][t] - hvac_power[n][h][t] * self.thermal_resist[n][h] -
										hvac_temp[n][h][t - 1]) * math.exp(
										- self._delta_t / (self.thermal_resist[n][h] * self.thermal_cap[n][h])))
										- self._big_m * (1 - hvac_mode_cool[n][h][t])), f'HVAC_Cooling_LB_state' + increment

						self.milp += hvac_active[n][h][t] == hvac_mode_heat[n][h][t] + hvac_mode_cool[n][h][t], f'HVAC_Active_Link_' + increment
						self.milp += hvac_power[n][h][t] == hvac_active[n][h][t] * self.hvac_capacity[n][h], f'HVAC_On_Off_' + increment
					...
					self.milp += hvac_temp[n][h][t] - self.hvac_temp_max[n][h] <= hvac_cost_comfort[n][h][t], f'HVAC_Cost_Above_UB' + increment
					self.milp += hvac_temp[n][h][t] - self.hvac_temp_min[n][h] >= -hvac_cost_comfort[n][h][t], f'HVAC_Cost_Below_UB' + increment
	...
	def generate_outputs(self) -> OutputsS3PoolDict:
		...
		# EVs_____________________________________________________________________________________________
		# Required when vars include "-" since puLP converts it to "_"
